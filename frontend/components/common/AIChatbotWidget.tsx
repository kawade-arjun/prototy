...
export const AIChatbotWidget: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
      {/* Chat Window Panel */}
      {isOpen && (
        <div className="mb-3 w-80 sm:w-96 max-h-[550px] h-[500px] rounded-3xl border border-neutral-800 bg-neutral-900/95 shadow-2xl backdrop-blur-xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
          ...
          <div className="p-4 border-b border-neutral-800 bg-neutral-950 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-amber-400 border-2 border-neutral-950" />
              </div>
              <div>
                ...
                <p className="text-[10px] text-neutral-400 font-mono">Real-time Platform Assistant</p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 text-xs">
                ...
                <div
                  className={`max-w-[85%] p-3.5 rounded-2xl leading-relaxed whitespace-pre-wrap ${
                    msg.sender === 'user'
                      ...
                      : 'bg-neutral-800 text-neutral-200 border border-neutral-700/80 rounded-bl-none shadow-sm'
                  }`}
                >
                  {msg.text}
                </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2 max-w-[90%]">
                    {msg.suggestedActions.map((action, i) => (
                      <button
                        ...
                        className="px-2.5 py-1 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 hover:bg-amber-500/20 text-[10px] font-semibold transition-all text-left"
                      >
                        ⚡ {action}
                      </button>
                    ))}
            ...
            {/* Typing Indicator */}
            {isTyping && (
              <div className="flex items-center gap-2 p-3 rounded-2xl bg-neutral-800 border border-neutral-700/80 text-neutral-400 text-xs w-max">
          ...
          >
            <input
              ...
              disabled={isTyping}
              className="flex-1 px-3.5 py-2 rounded-xl bg-neutral-900 border border-neutral-800 text-xs text-white placeholder-neutral-500 focus:outline-none focus:border-amber-500 transition-colors"
            />
            <button
              type="submit"
