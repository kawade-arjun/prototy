    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row items-start gap-6">
        ...
        <aside className="w-full lg:w-56 xl:w-54 shrink-0 lg:sticky lg:top-20 z-20 space-y-3 text-xl font-bold">
          ...
          <div className={`bg-white dark:bg-[#0c101c] rounded-2xl p-2.5 border border-slate-200/80 dark:border-white/[0.08] shadow-md flex-col gap-1 transition-all ${
            isMobileMenuOpen ? 'flex' : 'hidden lg:flex'
          }`}>
            <div className="px-3 py-2 flex items-center justify-between border-b border-slate-100 dark:border-white/[0.06] mb-1.5">
              <span className="text-[10px] font-mono font-extrabold uppercase tracking-widest text-slate-400 dark:text-slate-500 flex items-center gap-1.5">
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl text-xs font-bold transition-all duration-150 group text-left ${
                      isActive
                        ? 'bg-slate-900 text-white dark:bg-amber-600 dark:text-white font-extrabold shadow-sm ring-1 ring-amber-500/40'
                  ...
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className={`shrink-0 transition-colors ${
                        isActive ? 'text-amber-400 dark:text-white' : 'text-slate-400 group-hover:text-amber-500 dark:group-hover:text-amber-400'
                      }`}>
                        {tab.icon}
                      </span>
                    ...
                    {isActive && (
                      <span className="w-1.5 h-4 bg-amber-500 dark:bg-white rounded-full shrink-0 shadow-xs" />
                    )}
                  </button>
                );
              })}
